...
for i in $(seq 1 "$ITERATIONS"); do
    ...
    create_us=$(perf_time_us sudo "$NS_RUNTIME_BIN" create --bundle="$NS_TEST_BUNDLE" "$id")
    start_us=$(perf_time_us sudo "$NS_RUNTIME_BIN" start "$id")
    delete_us=$(perf_time_us sudo "$NS_RUNTIME_BIN" delete "$id")
